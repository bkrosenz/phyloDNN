...
class GATv2Conv(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int, heads: int = 1,
                 ...
                 bias: bool = True, share_weights: bool = False, node_dim: int = 0, **kwargs):
        super(GATv2Conv, self).__init__(node_dim=node_dim, **kwargs)
        ...
        self.lin_l = torch.nn.Conv1d(in_channels, heads * out_channels, bias=bias,
                                     weight_initializer='glorot')
        if share_weights:
            self.lin_r = self.lin_l
        else:
            self.lin_r = torch.nn.Conv1d(in_channels, heads * out_channels, bias=bias,
                                         weight_initializer='glorot')
