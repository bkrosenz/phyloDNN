...
class EdgeConvNet(GraphNetwork):
    def __init__(self,
                 ...
                 device='cpu'):
        '''Embedding layer followed by resnet layer followed by EdgeConv layers'''
        super().__init__()
        self.dims = [gene_embedding] + hidden_channels
        self.ks = ks
        self.edge_net_depth = edge_net_depth
        self.device = device
        ...
        for i, (k, out_dim) in enumerate(zip(ks, hidden_channels)):
            layers = [fc_layer(2*in_dim, 2 * in_dim, graph_batch_norm)
                      for _ in range(edge_net_depth-1)]
            edge_mapper = nn.Sequential(
                *layers, fc_layer(2 * in_dim, out_dim, graph_batch_norm)
            )
            if k is not None:
...
class RNNEncoder(Module):
    ...
    def __init__(self, num_rnn_layers, pretransform, pretransform_output_dim, output_size):
        '''simple LSTM encoder for sequences'''
        super().__init__()
...
class KnnGraphEncoder(Module):
    ...
    def __init__(self,
        ...
        '''dynamic edge conv (create new graphs using k nearest neighbors)'''
        super().__init__()
...
class DeepSet(Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_size=25, num_layers=4):
        super().__init__()
        layers = [nn.Linear(in_channels, hidden_size), nn.ReLU(inplace=True)]
