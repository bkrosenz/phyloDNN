...
def get_last_checkpoint(dir_name: Path):
    checkpoints = (p for p in dir_name.rglob("*.ckpt") if not p.name.startswith("."))
    pretrained_filename = max(checkpoints, key=lambda x: x.stat().st_ctime)
    return pretrained_filename


def squareform(arr: torch.Tensor, n:int|None=None, add_diagonal=False):
    """convert list of upper diagonal entries to symmetric matrix, optional initial batch dimension.
    Idempotent.

    Args:
        arr (Tensor): input array of shape npairs or nbatches x npairs
        n (int, optional): size. if not specified will be inferred. Defaults to None.
    ...
    if b:
        bix = (
            torch.arange(b, dtype=ix.dtype).repeat_interleave(ix.shape[-1]).unsqueeze(0)
        )
        ix = torch.concat((bix, ix.repeat((1, b))))
        x = torch.sparse_coo_tensor(
            ix.to(arr.device), arr.ravel(), size=(b, n, n)
        ).to_dense()
    else:
        x = torch.sparse_coo_tensor(ix.to(arr.device), arr, size=(n, n)).to_dense()
...
def validation_step(
    ...
    for l in b.unique():
        ...
        try:
            if dtype == "covariance":
                ytrue_batch = covariance_to_distance(ytrue_batch.numpy())
            ...
            true_tree = njtree(tuple(map(tuple, ytrue_batch)))
            pred_tree = njtree(
                tuple(map(tuple, ypred_batch)), true_tree.taxon_namespace
            )
        ...
        if hamming_nj:
            h_tree = njtree(
                torch.pdist(data.x[idx].squeeze().float(), p=1),
...
def validation_step_list(
    ...
    rf_tails = uw_dist_total = w_dist_total = ynorm_total = n = zero_preds = 0.0
    for y, ypred in zip(data.y, model(data)):
        ...
        rf_p, rf_tail = yh_prob(
            true_tree, rf_distance(pred_tree, true_tree, normalize=False)
        )
        print(
...
def summarize(
    ...
    try:
        ...
        if n_batches is None:
            ...
            vals = [val_step(model, data) for data in islice(data_loader, n_batches)]
...
def load_train_state(file_path, model, opt, device, opt_cpu=None):
    """NOTE: to change device of model, must create via the constructor, not loading a torch.save'd file"""
    # , map_location=device) # need map location if saved from different device
    checkpoint = load(file_path, map_location=device)  # torch.device('cpu'))
    model.load_state_dict(checkpoint["model_state_dict"])
    ...
    if opt_cpu is not None:
        opt_cpu = opt_cpu.load_state_dict(checkpoint["optimizer_cpu_state_dict"])
...
def make_network(network, preprocessor=None, **kwargs):
    try:
        from phyloDNN.equivariant_nets import (
            AttentionNet,
            FreqNet,
            FreqNet2,
            FreqPairNet,
    ...
    if network == "small":
        ...
        raise NotImplementedError(
            f"""network {network} not implemented.  
            Please choose from: 
            compressed_attention, sdpa, local, equivariant, seq, dual, attention, freq-net, freq, freq-pair"""
        )
    return net


def build_optimizer(model, lr: float = 0.001, weight_decay=0.01):
    from torch.optim import AdamW

    optimizer = AdamW(
        params=model.parameters(), lr=lr, weight_decay=weight_decay, eps=1e-5
    )
    return optimizer
